...
export default function ChatScreen({ patient, labResults, initialMessage, onMessageConsumed }: ChatScreenProps) {
  ...
  // Handle Initial Message from Report Analysis
  useEffect(() => {
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      ...
      const errorMsg: Message = {
        id: (Date.now() + 1).toString(),
        role: 'ai',
        content: "I'm having trouble connecting to my clinical brain right now. This usually happens if the AI model is at capacity. Please try again in a moment.",
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
  ...
  return (
    <div className="h-[calc(100vh-140px)] lg:h-[calc(100vh-40px)] flex overflow-hidden">
      ...
      <section className="flex-1 flex flex-col bg-surface overflow-hidden">
        ...
        {/* Input Bar */}
        <div className="bg-white/80 backdrop-blur-md border-t border-outline-variant p-4 shrink-0">
